...
n_families <- length(unique(allometric_models$family, na.rm=T))
n_genera <- length(unique(allometric_models$genus, na.rm=T))
...
`r n_families` taxonomic families and `r n_genera` taxonomic genera.
...
Here we see that three models are available from the Brackett (1977) report.
We will select the third model using `select_model`.
...
Using the standard output of `df_mod` we obtain a summary of the model form,
the response variable, the needed covariates and their units, a summary of
the model specification (i.e., what makes the model unique within the
...
We can see here that `df_mod` will require two covariates called `dsob`, which
