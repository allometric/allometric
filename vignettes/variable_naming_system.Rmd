...
The variable naming system attempts to provide a standard set of names that can
clarify the use of the variable while remaining succinct and easy to write.
In the variable naming system, the diameter outside bark at breast height is
referred to as **dsob**. Let us inspect what this name means further.
...
`r nrow(get_measure_defs())` established measures:
...
component. There are `r nrow(get_component_defs())` established components:
