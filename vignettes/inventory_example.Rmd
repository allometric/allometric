...
Before predicting, it is useful to inspect the covariates required by the
models in our set. The `model_call` function reveals these.
...
function call for each model we have specified, using the standardized
covariates `dsob` (diameter of the stem outside bark at breast height) and
`hst` (total height of the stem).

Using `final_set`, we can easily merge the models to our tree-list and make
predictions. Predictions are made by passing the `model` column to `predict`,
along with the covariate values, matched to each model by covariate name.

```{r}
tree_vols <- final_set %>%
  left_join(fia_trees, by = "SPCD") %>%
  mutate(vol_m3 = predict(model, dsob = DIA * 2.54, hst = HT * 0.3048)) %>%
  mutate(vol_cuft = units::set_units(vol_m3, "ft^3"))

head(tree_vols)
```

`predict` takes each model in the `model` column and applies it to the
covariate values assigned in the function call. Because the covariates are
matched by name, the order of the arguments does not matter. Aggregating to
the plot-level we obtain
...
and, potentially, their order. Because `predict` matches covariates to each
model by name, this does not complicate prediction: models simply use the
covariates they require, in whatever order.
...
the remaining models take `dsob` and `hst` arguments.

### Making Biomass Predictions with Different Model Forms

We merge the models to the tree list and predict, passing both covariates to
`predict`. The *Abies concolor* model uses only `dsob`; the remaining models
use `dsob` and `hst`. Each model is applied to its own rows.

```{r}
tree_agbs <- biomass_models %>%
  left_join(fia_trees, by = "SPCD") %>%
  mutate(agb_kg = predict(model, dsob = DIA * 2.54, hst = HT * 0.3048))
...
`predict`) it will be necessary to run `library(allometric)` to gain access
to those functions.