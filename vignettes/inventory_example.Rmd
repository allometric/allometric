...
We can see that `stemvol_mods` contains a huge amount of volume models. That is
because `allometric` is global in scope. Most of these models are not relavent
for our needs. Next, we will drill down and select models that were developed
with data from `US-OR` (the state of Oregon, United States).
...
```{r}
final_set %>% mutate(call = model_call(model_call[[1]]))
```
