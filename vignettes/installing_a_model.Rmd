...
Once a publication and added models are located in a `.R` file in the
`inst/publications` the models contained within can now be installed
by the user using `install_models()`. In fact, this is exactly how the user
initially gains access to the `allometric_models` table. If you are ever 
interested in viewing the source code for a model, just find the matching 
`.R` file in `inst/publications`, which can be viewed locally or on GitHub. 
The files used for this example are located at
`inst/publications/temesgen_2008.R` and `inst/publications/brackett_1977.R`,
respectively.
