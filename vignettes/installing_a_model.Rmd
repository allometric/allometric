...
`model_description` and `predict_fn`.
...
- `model_description` - a named list containing model descriptors. In this case
...
head(model_descriptions)
```

We can see that `model_descriptions` contains a number of individual models
