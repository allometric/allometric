...
construct_lambert_set <- function(category, b_params, b_params_names,
...
#for(i in seq_along(diaht_response_funcs)) {
#  category <- names(diaht_response_funcs)[[i]]
#
#  response_unit <- list()
#  response_unit[[response_unit_defs[[category]]]] <- list(
#    units::as_units("kg")
#  )
#
#  parameter_names <- diaht_b_params_names[grepl(category, diaht_b_params_names, fixed=T)]
#
#  model_specifications <- diaht_b_params %>%
#    dplyr::filter(parameter %in% parameter_names, !is.na(genus)) %>%
#    tidyr::pivot_wider(names_from = parameter, values_from = estimate) %>%
...
#  set <- FixedEffectsSet(
#    response_unit = response_unit,
#    covariate_units = diaht_covariate_units,
#    parameter_names = parameter_names,
#    predict_fn = diaht_response_funcs[[category]],
#    model_specifications = model_specifications
#  )
#
#  lambert_2005 <- lambert_2005 %>% add_set(set)
